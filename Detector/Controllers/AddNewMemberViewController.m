...
@interface AddNewMemberViewController () <UITableViewDelegate, UITableViewDataSource, UITextFieldDelegate, UIImagePickerControllerDelegate,UINavigationControllerDelegate>
...
@property (nonatomic, copy) NSString *isMale;//(1、默认 2、选中，其他为初始状态)
...
- (void)saveButtonClick {
    ...
    if ([self.testModel isEqualToString:@"1"]) {//外部人员
        if (!self.name.length) {
            ...
            HUD_SHOW(@"", @"");
            params = [NSMutableDictionary dictionaryWithObjectsAndKeys:self.name, @"staffName", self.IDCard, @"idNumber", @"1", @"staffType", nil];
        }
    }else if ([self.testModel isEqualToString:@"2"]) {//内部员工
        if (!self.name.length) {
            HUD_TEXTONLY(@"请填写姓名");
            return;
            ...
            HUD_SHOW(@"", @"");
            params = [NSMutableDictionary dictionaryWithObjectsAndKeys:self.name, @"staffName", @"0", @"staffType", self.serialNumber, @"serialNumber", nil];
            if (self.IDCard.length) {//身份证
                [params setValue:self.IDCard forKey:@"idNumber"];
            }
        }
    }
    ...
    [[NetRequest sharedRequest] postURLAnotherWay:[Add_Member_Message getFullRequestPath] parameters:params success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary<NSString *,id> *)info {
    ...
    if (self.isSelectFaceImage) {
        ...
        [self postImageWithImage:self.faceImage isFaceImage:YES];
    }else {
        self.fingerImage = [UIImage imageWithData:data];
        [self postImageWithImage:self.fingerImage isFaceImage:NO];
    }
    [self.tableView reloadData];
    [self dismissViewControllerAnimated:YES completion:nil];
}

- (void)postImageWithImage:(UIImage *)image isFaceImage:(BOOL)isFaceImage {
    NSDictionary *params = @{
                             @"category": [NSString stringWithFormat:@"%d", isFaceImage]
                             };
    [[NetRequest sharedRequest] uploadImageWithURL:[Post_Image getFullRequestPath] parameters:params imageKey:@"file" image:image success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
