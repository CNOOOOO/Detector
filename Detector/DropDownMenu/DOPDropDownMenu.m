...
- (instancetype)initWithOrigin:(CGPoint)origin andHeight:(CGFloat)height {
    ...
    if (self) {
        ...
        bottomShadow.backgroundColor = [UIColor colorWithWhite:0 alpha:0.1];
...
- (void)animateTableView:(UITableView *)tableView show:(BOOL)show complete:(void(^)(void))complete {
    if (show) {
        ...
        [UIView animateWithDuration:0.3 animations:^{
            self->_tableView.frame = CGRectMake(self.origin.x, self.frame.origin.y + self.frame.size.height, self.frame.size.width, tableViewHeight);
        }];
    } else {
        [UIView animateWithDuration:0.3 animations:^{
            self->_tableView.frame = CGRectMake(self.origin.x, self.frame.origin.y + self.frame.size.height, self.frame.size.width, 0);
        } completion:^(BOOL finished) {
...
- (void)confiMenuWithSelectRow:(NSInteger)row {
    ...
    /***************这部分只是为了满足项目中的需求，其他项目可以不要***************/
    if (_currentSelectedMenudIndex == 0) {
        CATextLayer *title = (CATextLayer *)_titles[_currentSelectedMenudIndex + 1];
        if (row == 2) {
            title.string = @"无";
        }else {
            title.string = @"添加人员";
        }
        [self animateIdicator:_indicators[_currentSelectedMenudIndex + 1] background:_backGroundView tableView:_tableView title:_titles[_currentSelectedMenudIndex + 1] forward:NO complecte:^{
            self->_show = NO;
        }];
        [(CALayer *)self.bgLayers[_currentSelectedMenudIndex + 1] setBackgroundColor:[UIColor whiteColor].CGColor];
        
        CAShapeLayer *indicator = (CAShapeLayer *)_indicators[_currentSelectedMenudIndex + 1];
        indicator.position = CGPointMake(title.position.x + title.frame.size.width / 2 + 8, indicator.position.y);
    }
    /***************这部分只是为了满足项目中的需求，其他项目可以不要***************/
}
